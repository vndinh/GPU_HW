...
__global__ void matrixMulBreadth(int *A, int *B, int *C, int size) {
	int tid, tx, ty;
...
int main() {
	...
	// Matrix Multiplication using streams with breadth-first approach
	for (int i = 0; i < NSTREAM; i++) {
		int ioffset = i * iElem;
		matrixMulBreadth<<<grid, block, 0, stream[i]>>>(&d_A[ioffset], d_B, &d_C[ioffset], nx);
