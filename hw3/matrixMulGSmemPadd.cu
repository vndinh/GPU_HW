...
__global__ void matrixMulGSmemPadd (int *A, int *B, int *C, int size) {

	// Static shared memory
	__shared__ int ds_A[TILE_WIDTH][TILE_WIDTH+IPAD];
	__shared__ int ds_B[TILE_WIDTH][TILE_WIDTH+IPAD];
...
int main() {
	...
	matrixMulGSmemPadd<<<grid, block, TILE_WIDTH*(TILE_WIDTH+1)*sizeof(int)>>>(d_A, d_B, d_C, nx);
